...
R_beta_b = (1-Rcoeff_mean)/Rcoeff_mean * R_beta_a[2]

YR = Beta(R_beta_a[2], R_beta_b)
YRpdf = pdf.(YR,collect(0:0.01:1.0))
cRpdf = sum(YRpdf)
nYR = YRpdf/cRpdf

plt.plot(collect(0:0.01:1.0),nYR)
plt.xlim([0,1.0])
plt.show()
...
Threads.@threads for ii in 1:sim_ens
    #repd_ii = repd[ii]
    Ivect[:,ii], Revect[:,ii], Lvect[:,ii], Lcvect[:,ii], Dvect[:,ii], Svect[:,ii], cvect[:,ii], Rewvect[:,ii], policy[:,ii], Rest[:,ii], R0est[:,ii] = EpiRun_preds_noS_R_uncertain(Ivect[:,ii], Revect[:,ii], Lvect[:,ii], Lcvect[:,ii], Ldvect[:,ii], Dvect[:,ii], Svect[:,ii], cvect[:,ii], Rewvect[:,ii], policy[:,ii], Rest[:,ii], R0est[:,ii], nY, nYd, reward, par)
end
