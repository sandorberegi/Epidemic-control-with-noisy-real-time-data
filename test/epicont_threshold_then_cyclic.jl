...
len_nothing = 7#3
I_start = 0
cyclic_mode = 0 #0: free, before I reaching theshold, 1: cyclic ctrl on
cyc_start_day = 98+15*3-1 #13*3-1
...
rf = 3 #days
R_est_wind = 5 #window for R estimation
use_S = 0
...
#init policies
for ii in 1:(rf*(len_LD+len_nothing)):(ndays+pred_days)
    for jj in 1:len_nothing*rf
        if ii+jj <= ndays+pred_days
        policy[ii+jj,:].=1
        end
    end

    for jj in 1:len_LD*rf
        if len_nothing*rf+ii+jj <= ndays+pred_days
...
Threads.@threads for ii in 1:sim_ens
    #repd_ii = repd[ii]
    Ivect[:,ii], Revect[:,ii], Lvect[:,ii], Lcvect[:,ii], Dvect[:,ii], Svect[:,ii], cvect[:,ii], Rewvect[:,ii], policy[:,ii], Rest[:,ii], R0est[:,ii] = EpiRun_threshold_cyclic(Ivect[:,ii], Revect[:,ii], Lvect[:,ii], Lcvect[:,ii], Ldvect[:,ii], Dvect[:,ii], Svect[:,ii], cvect[:,ii], Rewvect[:,ii], policy[:,ii], Rest[:,ii], R0est[:,ii], nY, nYd, reward, par)
end
