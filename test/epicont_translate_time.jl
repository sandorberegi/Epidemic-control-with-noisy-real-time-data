...
# ρ, ρvar = 0.25, 8.0 #Under reporting, mean/variance
...
##
Ivect, Revect, Lvect, Lcvect, Dvect, Svect, cvect, Rewvect, policy, Rest, R0est = Ivect[99:end,:], Revect[99:end,:], Lvect[99:end,:], Lcvect[99:end,:], Dvect[99:end,:], Svect[99:end,:], cvect[99:end,:], Rewvect[99:end,:], policy[99:end,:], Rest[99:end,:], R0est[99:end,:]
...
cd("figs4/ebola_real14_later_bin")
...
for ii in 1:sim_ens
    ...
    n_vect[ii] = count(x->(x.==1), policy[1:ndays-tr,ii])
    s_vect[ii] = count(x->(x.==2), policy[1:ndays-tr,ii])
    l_vect[ii] = count(x->(x.==3), policy[1:ndays-tr,ii])
